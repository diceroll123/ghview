    ...
    match DrawMode::from_app(app) {
        ...
        DrawMode::DetailFrontpage => {
            if app.direct_source {
                let cols = Layout::horizontal([Constraint::Length(4), Constraint::Fill(1)])
                    .split(main_area);
                panels::draw_repos_strip(f, app, cols[0]);
                panels::draw_repo_frontpage(f, app, cols[1]);
            } else {
                let cols = Layout::horizontal([
                    Constraint::Length(4),
                    Constraint::Length(4),
        ...
        DrawMode::PreviewPrList => {
            if app.direct_source {
                let cols = Layout::horizontal([Constraint::Fill(38), Constraint::Fill(47)])
                    .split(main_area);
                panels::draw_source_prs(f, app, cols[0]);
                panels::draw_pr_detail(f, app, cols[1]);
            } else {
        ...
        DrawMode::PreviewIssueList => {
            if app.direct_source {
                let cols = Layout::horizontal([Constraint::Fill(38), Constraint::Fill(47)])
                    .split(main_area);
                panels::draw_source_issues(f, app, cols[0]);
                panels::draw_issue_detail(f, app, cols[1]);
            } else {
        ...
        DrawMode::PreviewRepoList => {
            if app.direct_source {
                let cols = Layout::horizontal([Constraint::Fill(38), Constraint::Fill(47)])
                    .split(main_area);
                panels::draw_repos(f, app, cols[0]);
                match app.repo_view {
                    RepoView::Frontpage => panels::draw_repo_frontpage(f, app, cols[1]),
                    RepoView::Issues => panels::draw_issues(f, app, cols[1]),
                    RepoView::Prs => panels::draw_prs(f, app, cols[1]),
                }
            } else {
                let (src, repos, prs) = preview_pcts(app.focus);
                let cols = Layout::horizontal([
...
#[inline]
pub(super) fn relative_time(iso: &str, now: jiff::Timestamp) -> String {
    use jiff::Timestamp;
...
mod tests {
    ...
    fn fixed_now() -> Timestamp {
        "2026-01-15T12:00:00Z".parse().expect("valid timestamp")
    }

    /// Build a fake ISO-8601 timestamp that is `secs` seconds before `fixed_now`.
    ...
    fn relative_time_28_days_is_weeks_not_zero_months() {
        ...
        assert_eq!(relative_time(&ago(2_419_200), fixed_now()), "4w");
    }

    #[test]
    fn relative_time_30_days_is_one_month() {
        // 30 days = 2,592,000 secs — the first value that should produce "1mo".
        assert_eq!(relative_time(&ago(2_592_000), fixed_now()), "1mo");
    }
}
